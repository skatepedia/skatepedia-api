...
class SkateVideoSiteSpider(CrawlSpider):
    ...
    rules = (
        Rule(LinkExtractor(allow=("/page/\d+")), callback='parse_archive'),
    )
    ...
    def parse_video(self, response):
        ...
        info_keys = [remove_tags(column) for column in response.css('.videoinfo tr > td:nth-child(1)').extract()]
        info_values = [remove_tags(column) for column in response.css('.videoinfo tr > td:nth-child(2)').extract()]
